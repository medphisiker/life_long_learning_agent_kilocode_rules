...
- `NetrunnerDeck.tsx` — Универсальный хаб управления. Контекстно-зависим:
    ...
    - **Функциональность (v3.1)**: Поддерживает изменение высоты (resizable) и сворачивание (minimize) до 40px кнопкой управления.
- `UnifiedChat.tsx` — Единый компонент рендеринга диалогов. Используется в `ChatInterface`, `QuizHub` и `AlgoLab` для обеспечения консистентности визуализации (Markdown, KaTeX, RAG-карточки).
